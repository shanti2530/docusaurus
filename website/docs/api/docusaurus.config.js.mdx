...
#### Example Configuration for i18n Sites

To reject broken links for the main language while ignoring them for translated versions, you can configure the `i18n.localeConfigs` as follows:
...
This setup ensures that the main language (`en`) throws errors for broken links, while translated languages (`fr`, `de`) ignore them. Adjust your configuration to match the locales you support and your desired handling behavior.
